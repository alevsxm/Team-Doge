...
describe Position do
  let(:alex_positions) { Position.create_positions(response_hash) }
  it 'has the right number of positions' do
    expect(alex_positions.length).to eq(5)
  end
  it 'has the current position name' do
    expect(alex_positions[0].company_name).to eq("General Assemb.ly")
  end
  it 'has the a past position as not current' do
    expect(alex_positions[1].is_current).to eq(false)
  end
end
